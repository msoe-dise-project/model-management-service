...
if __name__ == "__main__":
    ...
    if args.object == "project":
        ...
        if args.action == "get":
            get_model_test(base_url, args.id)
        elif args.action == "create":
            try:
                if args.metrics.endswith(".json"):
                    ...
                    with open(args.metrics, 'r', encoding='utf-8') as file:
                        ...
                        model_payload = {"project_id": args.project_id,
                                         ...
                                         "test_timestamp": args.timestamp,
                                         "test_metrics": metrics,
                                         "passed_testing": args.test_passed}
                        create_model_test(base_url, model_payload)
                else:
                    print("Specified file must be json", file=sys.stderr)
                    sys.exit(1)
            except json.decoder.JSONDecodeError:
